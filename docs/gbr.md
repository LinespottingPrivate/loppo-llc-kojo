...
iOS/Android app spectates (and can inject into) the desktop coding-CLI session
...
kojo's Tailscale tsnet PTY UI (default **:8080**) **stays**. That is the
mobile terminal remote. Build Remote Agent is a **separate** pairing device:
store app + GitHub/HTTPS relay. Do **not** mix `kojo --peer` with `gbr/1`.
